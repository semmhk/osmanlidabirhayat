...
- [x] `OlayYukleyici` 6 dosya desteği (`kurulus.json`, `yukselme.json`, `duraklama.json`, `gerileme.json`, `dagilma.json` & **`gundelik_hayat.json`**), tarih yılı min/max ve `alt_donem` şema doğrulayıcısı ile yazıldı (`lib/motor/olay_yukleyici.dart`).
