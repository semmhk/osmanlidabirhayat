...
- [x] **Padişah Tuğraları Gösterimi Tamamlandı**: Wikimedia Commons veritabanından 35 padişah için kamu malı (PD-Ottoman) gerçek tuğra PNG görselleri indirilerek `assets/tugralar/` altına kaydedildi. `assets/tugralar/CREDITS.md` dosyası oluşturuldu. Osman Gazi ve Fetret Devri için uydurma görsel kullanılmadı (null fallback). `PadisahDeposu` tüm 36 padişah ile güncellendi. `OyunEkrani` üst bilgi barına tuğra görselleri (42px yükseklik) eklendi. Unit test (`test/padisah_tugra_test.dart`) ile 16/16 test başarıyla pas geçti.
