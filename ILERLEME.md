...
- [x] **OlumEkrani & Nesil Devamı Entegrasyonu Tamamlandı**: `lib/ekranlar/oyun_ekrani.dart` ve `lib/main.dart` oluşturularak ölüm anında `OlumEkrani` ekranına geçiş sağlandı. `OyunMotoru.nesilDevamEt(Cocuk)` metodu yazılarak seçilen çocuğun yaş/takvim yılı hesabı (`dogumYili = karakter.dogumYili + cocuk.ebeveynYasiDogum`), %40 servet miras aktarımı, bayrak/meslek temiz sayfası ve egzoz olay yönetimi sağlandı. Widget/entegrasyon testi (`test/nesil_devami_entegrasyon_test.dart`) yazılarak 13/13 test ile doğrulandı.
